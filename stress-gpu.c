...
static const stress_help_t help[] = {
	...
	{NULL, "gpu-tex-size N", "specify upload texture SIZExSIZE"},
...
void gles2_init(uint32_t width, uint32_t height, int frag_n, GLsizei texsize)
{
	...
	if (glGetError() != GL_NO_ERROR)
		(void)fprintf(stderr,
			      "ERROR: Failed to get the storage location of %d\n",
			      frag_n);
	...
	if (texsize > 0) {
		...
		if (texsize > maxsize)
			(void)fprintf(stderr,
				      "ERROR: Image size exceeds max texture size \n");
...
EGLConfig get_config(void)
{
	...
	if (eglGetConfigs(display, NULL, 0, &num_configs) == EGL_FALSE)
		(void)fprintf(stderr, "EGL: There are no EGL configs \n");

	EGLConfig *configs = malloc(num_configs * sizeof(EGLConfig));
	if ((eglChooseConfig(display, egl_config_attribs,
			     configs, num_configs,
			     &num_configs) == EGL_FALSE) || (num_configs == 0))
		(void)fprintf(stderr, "EGL: can't choose EGL config \n");
...
static void egl_init(uint32_t size_x, uint32_t size_y)
{
	...
	if (fd < 0)
		(void)fprintf(stderr, "ERROR: couldn't open %s, skipping\n",
			      devicenode);

	gbm = gbm_create_device(fd);
	if (gbm == NULL)
		(void)fprintf(stderr, "ERROR: couldn't create gbm device \n");

	if ((display =
	     eglGetPlatformDisplay(EGL_PLATFORM_GBM_KHR, gbm,
				   NULL)) == EGL_NO_DISPLAY)
		(void)fprintf(stderr,
			      "EGL: eglGetPlatformDisplay failed with vendor \n");

	EGLint majorVersion;
	EGLint minorVersion;
	if (eglInitialize(display, &majorVersion, &minorVersion) == EGL_FALSE)
		(void)fprintf(stderr, "EGL: Failed to initialize EGL \n");
	...
	if (!gs)
		(void)fprintf(stderr, "ERROR: Could not create gbm surface \n");

	if ((surface =
	     eglCreatePlatformWindowSurface(display, config, gs,
					    NULL)) == EGL_NO_SURFACE)
		(void)fprintf(stderr, "EGL: Failed to allocate surface \n");
	...
	if ((context =
		...
		(void)fprintf(stderr, "EGL: Failed to create context \n");

	if (eglMakeCurrent(display, surface, surface, context) == EGL_FALSE)
		(void)fprintf(stderr, "EGL: Failed to make context current \n");
}
